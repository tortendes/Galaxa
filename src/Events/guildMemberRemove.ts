...
export const event: Event = {
    ...
    run: (client, member) => {
        ...
        loadImage('../Galaxa/src/Assets/background.png').then((image) => {
            ...
            ctx.font = applyText(canvas, `${member.user.tag} left`, 'Inter-Bold', 'bold')
            ctx.fillStyle = '#fff'
        ...
        loadImage(member.user.displayAvatarURL({ format: 'png' })).then((image) => {
            ...
            const attachment = new MessageAttachment(canvas.toBuffer(), 'bye.png')

            const embed = client.embed({
                color: '#4ae9f7',
                image: {
                    url: 'attachment://bye.png'
                }
            })

            member.guild.channels.cache.get('695231971676258347').send({ content: `<@${member.id}> (${member.tag}) has left.`, embeds: [embed], files: [attachment] })
        })
    }
}
